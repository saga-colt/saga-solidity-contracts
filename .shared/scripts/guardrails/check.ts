...
function ensureImmutableInstall(): boolean {
  ...
  if (result.status !== 0) {
    logger.error(
      "Lockfile verification failed. Run `yarn install --mode=update-lockfile` to regenerate the lockfile after syncing .shared.",
    );
    return false;
  }
...
if (require.main === module) {
  ...
  const eslintMaxWarnings =
    typeof opts.eslintMaxWarnings === "number" && !Number.isNaN(opts.eslintMaxWarnings) ? opts.eslintMaxWarnings : undefined;
  const solhintMaxWarnings =
    typeof opts.solhintMaxWarnings === "number" && !Number.isNaN(opts.solhintMaxWarnings) ? opts.solhintMaxWarnings : undefined;
