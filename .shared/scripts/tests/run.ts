...
test("configLoader falls back to shared defaults when project override is missing", () => {
  ...
  assert.equal(slitherConfig.json, "reports/slither-report.json", "shared slither config should expose expected report path");
});
...
test("validateConfig surfaces missing required keys and type mismatches", () => {
  ...
  assert.ok(
    wrongType.errors.some((error) => error.includes("Configuration 'threshold' has wrong type. Expected number, got string")),
