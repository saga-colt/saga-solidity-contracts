...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  for (const [assetAddress, _feed] of Object.entries(plainFeeds)) {
    ...
    console.log(`Set plain Tellor wrapper for asset ${assetAddress} to ${tellorWrapperAddress}`);
  }
  ...
  for (const [assetAddress, _config] of Object.entries(thresholdFeeds)) {
    await oracleAggregator.setOracle(assetAddress, tellorWrapperWithThresholdingAddress);
    console.log(`Set Tellor wrapper with thresholding for asset ${assetAddress} to ${tellorWrapperWithThresholdingAddress}`);
  }
...
func.tags = ["usd-oracle", "oracle-aggregator", "oracle-wrapper", "usd-tellor-wrapper"];
func.dependencies = [USD_TELLOR_ORACLE_WRAPPER_ID, USD_TELLOR_WRAPPER_WITH_THRESHOLDING_ID, USD_ORACLE_AGGREGATOR_ID];
