...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const { address: amoManagerAddress } = await hre.deployments.get(D_AMO_MANAGER_ID);

  await hre.deployments.deploy(D_ISSUER_CONTRACT_ID, {
    from: deployer,
    args: [collateralVaultAddress, tokenAddresses.D, oracleAggregatorAddress, amoManagerAddress],
    contract: "IssuerV2",
    autoMine: true,
    log: false,
  });
  ...
  const { address: issuerAddress } = await hre.deployments.get(D_ISSUER_CONTRACT_ID);

  // Grant MINTER_ROLE to the IssuerV2 contract so it can mint d
  const dContract = await hre.ethers.getContractAt("ERC20StablecoinUpgradeable", tokenAddresses.D);
...
func.tags = ["d"];
func.dependencies = [D_COLLATERAL_VAULT_CONTRACT_ID, D_TOKEN_ID, "usd-oracle", D_AMO_MANAGER_ID];
