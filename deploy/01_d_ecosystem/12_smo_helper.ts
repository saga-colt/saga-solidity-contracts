...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (missingConfigs.length > 0) {
    console.log(`⚠️  Skipping SMOHelper deployment - missing configuration values: ${missingConfigs.join(", ")}`);
  ...
  const smoHelperContract = await hre.ethers.getContractAt("SMOHelper", smoHelperDeployment.address, await hre.ethers.getSigner(deployer));

  // Check if the governance multisig has the OPERATOR_ROLE
  const operatorRole = await smoHelperContract.OPERATOR_ROLE();
  const hasOperatorRole = await smoHelperContract.hasRole(operatorRole, config.walletAddresses.governanceMultisig);
