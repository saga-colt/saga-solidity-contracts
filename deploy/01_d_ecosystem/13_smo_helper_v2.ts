...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const hasNewOperatorRole = await smoHelperV2Contract.hasRole(operatorRole, newOperatorAddress);
  ...
  const hasGovernanceOperatorRole = await smoHelperV2Contract.hasRole(operatorRole, config.walletAddresses.governanceMultisig);
