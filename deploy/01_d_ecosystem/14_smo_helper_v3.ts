...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (missingConfigs.length > 0) {
    console.log(`⚠️  Skipping SMOHelper V2 deployment - missing configuration values: ${missingConfigs.join(", ")}`);
  ...
  const oldSmoHelperContract = await hre.ethers.getContractAt("SMOHelper", oldSmoHelper.address, await hre.ethers.getSigner(deployer));
  ...
  const hasOldOperatorRole = await oldSmoHelperContract.hasRole(operatorRole, oldOperatorAddress);
  ...
  try {
    const grantTx = await smoHelperV3Contract.grantRole(operatorRole, newOperatorAddress);
    await grantTx.wait();
  ...
  const hasNewOperatorRole = await smoHelperV3Contract.hasRole(operatorRole, newOperatorAddress);
  ...
  const hasGovernanceOperatorRole = await smoHelperV3Contract.hasRole(operatorRole, config.walletAddresses.governanceMultisig);
  ...
  const deployerHasAdminRole = await smoHelperV3Contract.hasRole(adminRole, deployer);
  ...
  const deployedUniswapRouterAddress = await smoHelperV3Contract.getUniswapRouter();
  ...
  if (!hasNewOperatorRole) {
    manualActionsNeeded.push(`❌ GRANT OPERATOR_ROLE to ${newOperatorAddress} on new SMOHelper V3 (${smoHelperV3Deployment.address})`);
  }
  ...
  if (!deployerHasAdminRole) {
    manualActionsNeeded.push(`❌ GRANT DEFAULT_ADMIN_ROLE to deployer (${deployer}) on new SMOHelper V3`);
  }

  // Check contract address configurations
  if (deployedDStableAddress !== dToken.address) {
    manualActionsNeeded.push(`❌ VERIFY dSTABLE address configuration on new SMOHelper V3`);
  }

  if (deployedRedeemerAddress !== dRedeemer.address) {
    manualActionsNeeded.push(`❌ VERIFY Redeemer address configuration on new SMOHelper V3`);
  }

  if (deployedUniswapRouterAddress !== config.uniswapRouter) {
    manualActionsNeeded.push(`❌ VERIFY Uniswap Router address configuration on new SMOHelper V3`);
  }
  ...
  if (manualActionsNeeded.length > 0) {
    ...
    console.log("   • Test the new SMOHelper V3 functionality after manual fixes");
  } else {
  ...
  console.log("\n💡 Additional Recommendations:");
  console.log("   • Update any frontend/backend integrations to use the new SMOHelper V3 address");
