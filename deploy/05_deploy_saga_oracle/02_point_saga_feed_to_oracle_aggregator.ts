...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (!sagaAddress) {
    console.log("SAGA token address not configured in network config. Skipping SAGA oracle aggregator wiring.");
    return true;
  }
  ...
  if (!thresholdFeeds[sagaAddress]) {
    console.log(`SAGA feed configuration not found for address ${sagaAddress}. Skipping oracle aggregator wiring.`);
    return true;
  }
  ...
  await oracleAggregator.setOracle(sagaAddress, tellorWrapperWithThresholdingAddress);

  console.log(`✅ Set Tellor wrapper with thresholding for SAGA asset ${sagaAddress} to ${tellorWrapperWithThresholdingAddress}`);

  console.log(`🔮 ${__filename.split("/").slice(-2).join("/")}: ✅`);
  return true;
};

func.tags = ["saga", "saga-oracle", "usd-oracle", "oracle-aggregator", "saga-oracle-wiring"];
func.dependencies = ["setup-saga-usd-tellor-oracle-feeds", USD_ORACLE_AGGREGATOR_ID];
