...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment): Promise<boolean> {
  ...
  console.log(`\n≻ Upgrading D Issuer to IssuerV2_2...`);

  await hre.deployments.deploy(D_ISSUER_V2_2_CONTRACT_ID, {
    from: deployer,
  ...
  const { address: newIssuerAddress } = await hre.deployments.get(D_ISSUER_V2_2_CONTRACT_ID);
  console.log(`   ↳ IssuerV2_2 deployed at ${newIssuerAddress}`);
  ...
  if (!(await dstable.hasRole(MINTER_ROLE, newIssuerAddress))) {
    await queueOrExecute(
      "Grant MINTER_ROLE to IssuerV2_2",
      async (): Promise<void> => {
    ...
    console.log("   ✓ IssuerV2_2 already has MINTER_ROLE");
  }
  ...
  if (pendingGovernance) {
    const flushed = await executor.flush("Upgrade D Issuer to IssuerV2_2");
    ...
    if (executor.useSafe) {
      ...
      console.log(`\n≻ 13_upgrade_issuer_v2_2/01_upgrade_issuer.ts: pending governance ⏳`);
      return false;
    }
  }

  console.log(`\n≻ 13_upgrade_issuer_v2_2/01_upgrade_issuer.ts: ✅`);
  return true;
};

func.tags = ["d", "issuer-v2_2", "upgrade"];
func.dependencies = [D_COLLATERAL_VAULT_CONTRACT_ID, D_TOKEN_ID, USD_ORACLE_AGGREGATOR_ID];
func.id = "upgrade-d-issuer-v2_2";
