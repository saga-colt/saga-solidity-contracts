...
import { DStakeCollateralVault, DStakeToken, DStakeRouterDLend, IDStableConversionAdapter, ERC20, IERC20 } from "../../typechain-types";
  ...
  describe(`DStakeCollateralVault for ${config.DStakeTokenSymbol}`, () => {
    ...
    beforeEach(async function () {
      ...
      if (!(await collateralVault.hasRole(routerRole, deployer.address))) {
        await collateralVault.connect(deployer).grantRole(routerRole, deployer.address);
      }
      ...
      expect(await collateralVault.hasRole(adminRole, deployer.address)).to.be.true;
    ...
    describe("Router Management (setRouter)", () => {
      it("Should only allow admin to set router", async function () {
        if (await collateralVault.hasRole(adminRole, user1.address)) {
          await collateralVault.connect(deployer).revokeRole(adminRole, user1.address);
        }
        await expect(collateralVault.connect(user1).setRouter(routerAddress)).to.be.revertedWithCustomError(
          collateralVault,
          "AccessControlUnauthorizedAccount",
        );

        await expect(collateralVault.connect(deployer).setRouter(routerAddress)).to.not.be.reverted;
      });

      it("Should revert if setting router to zero address", async function () {
        await expect(collateralVault.connect(deployer).setRouter(ZeroAddress)).to.be.revertedWithCustomError(
          collateralVault,
          "ZeroAddress",
        );
      });
      ...
      it("Should set and replace the router correctly, managing ROUTER_ROLE", async function () {
        ...
        await expect(collateralVault.connect(deployer).setRouter(newRouterAddress))
    ...
    describe("Recovery Functions", function () {
      ...
      describe("rescueToken", function () {
        it("Should successfully rescue non-restricted tokens", async function () {
          ...
          await expect(collateralVault.connect(deployer).rescueToken(mockTokenAddress, user1.address, testAmount))
        ...
        it("Should rescue partial balance", async function () {
          ...
          await expect(collateralVault.connect(deployer).rescueToken(mockTokenAddress, user1.address, partialAmount))
            .to.emit(collateralVault, "TokenRescued")
        ...
        it("Should revert when trying to rescue supported vault assets", async function () {
          ...
          await expect(collateralVault.connect(deployer).rescueToken(vaultAssetAddress, user1.address, 1n))
        ...
        it("Should revert when trying to rescue dStable token", async function () {
          ...
          await expect(collateralVault.connect(deployer).rescueToken(dStableTokenAddress, user1.address, testAmount))
            .to.be.revertedWithCustomError(collateralVault, "CannotRescueRestrictedToken")
        ...
        it("Should revert with zero address receiver", async function () {
          await expect(
            collateralVault.connect(deployer).rescueToken(mockTokenAddress, ZeroAddress, testAmount),
        ...
        it("Should handle rescue when token balance is insufficient", async function () {
          ...
          await expect(collateralVault.connect(deployer).rescueToken(mockTokenAddress, user1.address, excessiveAmount)).to.be.reverted;
        });
      });
      ...
      describe("rescueETH", function () {
        ...
        it("Should successfully rescue ETH", async function () {
          const receiverInitialBalance = await ethers.provider.getBalance(user1.address);
          const vaultInitialBalance = await ethers.provider.getBalance(collateralVaultAddress);

          await expect(collateralVault.connect(deployer).rescueETH(user1.address, ethAmount))
            .to.emit(collateralVault, "ETHRescued")
            .withArgs(user1.address, ethAmount);

          expect(await ethers.provider.getBalance(user1.address)).to.equal(receiverInitialBalance + ethAmount);
        ...
        it("Should revert with zero address receiver", async function () {
          await expect(collateralVault.connect(deployer).rescueETH(ZeroAddress, ethAmount)).to.be.revertedWithCustomError(
            collateralVault,
            "ZeroAddress",
          );
        });
        ...
        it("Should revert when contract has insufficient ETH", async function () {
          ...
          await expect(collateralVault.connect(deployer).rescueETH(user1.address, excessiveAmount))
        ...
        it("Should handle rescue when contract has no ETH", async function () {
          ...
          await collateralVault.connect(deployer).rescueETH(user1.address, ethAmount);

          // Try to rescue again when balance is 0
          await expect(collateralVault.connect(deployer).rescueETH(user1.address, 1n))
            .to.be.revertedWithCustomError(collateralVault, "ETHTransferFailed")
      ...
      describe("Integration tests", function () {
        it("Should rescue multiple different tokens", async function () {
          ...
          // Rescue second token
          await expect(collateralVault.connect(deployer).rescueToken(mockToken2Address, user1.address, testAmount))
            .to.emit(collateralVault, "TokenRescued")
        ...
        it("Should prevent rescue of newly added supported assets", async function () {
          ...
          if ((await router.vaultAssetToAdapter(vaultAssetAddress)) === ZeroAddress) {
            ...
            const vaultBalance = await vaultAssetToken.balanceOf(collateralVaultAddress);
            if (vaultBalance > 0n) {
              await expect(collateralVault.connect(deployer).rescueToken(vaultAssetAddress, user1.address, 1n)).to.not.be.reverted;
            }
