...
import { DStakeRouterDLend, DStakeCollateralVault, DStakeToken, IDStableConversionAdapter, ERC20, IERC20 } from "../../typechain-types";
  ...
  describe(`DStakeRouterDLend for ${config.DStakeTokenSymbol}`, function () {
    ...
    describe("Role-Based Access Control & Configuration", function () {
      it("admin can add a new adapter", async function () {
        await expect(router.connect(deployerSigner).addAdapter(vaultAssetAddress, adapterAddress))
      ...
      it("cannot add adapter with zero addresses", async function () {
        await expect(router.connect(deployerSigner).addAdapter(ZeroAddress, adapterAddress)).to.be.revertedWithCustomError(
          router,
          "ZeroAddress",
        );
        await expect(router.connect(deployerSigner).addAdapter(vaultAssetAddress, ZeroAddress)).to.be.revertedWithCustomError(
          router,
          "ZeroAddress",
        );
      });
      ...
      it("admin can remove an adapter", async function () {
        // First add
        await router.connect(deployerSigner).addAdapter(vaultAssetAddress, adapterAddress);
        // Then remove
        await expect(router.connect(deployerSigner).removeAdapter(vaultAssetAddress))
      ...
      it("admin can set defaultDepositVaultAsset", async function () {
        // Add adapter first
        await router.connect(deployerSigner).addAdapter(vaultAssetAddress, adapterAddress);
        await expect(router.connect(deployerSigner).setDefaultDepositVaultAsset(vaultAssetAddress))
      ...
      it("cannot set defaultDepositVaultAsset for unregistered asset", async function () {
        ...
        await expect(router.connect(deployerSigner).setDefaultDepositVaultAsset(nonVaultAsset)).to.be.revertedWithCustomError(
          router,
          "AdapterNotFound",
        );
      });
    ...
    describe("Deposit and Withdraw", function () {
      ...
      it("reverts when adapter under-delivers vault shares", async function () {
        ...
        await router.connect(deployerSigner).addAdapter(await mockVaultAsset.getAddress(), await udAdapter.getAddress());
        await router.connect(deployerSigner).setDefaultDepositVaultAsset(await mockVaultAsset.getAddress());
      ...
      it("captures surplus on positive slippage (bonus redeem adapter)", async function () {
        ...
        await router.connect(deployerSigner).addAdapter(bonusVaultAsset, bonusAdapterAddr);
        await router.connect(deployerSigner).setDefaultDepositVaultAsset(bonusVaultAsset);
    ...
    describe("Exchange Assets Using Adapters", function () {
      ...
      beforeEach(async function () {
        ...
        await dStableToken.connect(DStakeTokenSigner).approve(routerAddress, depositAmount);
        await router.connect(DStakeTokenSigner).deposit(depositAmount);
        await router.connect(deployerSigner).grantRole(await router.COLLATERAL_EXCHANGER_ROLE(), user1Addr);
      });
