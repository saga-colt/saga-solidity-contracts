...
export async function setupNewReserves(hre: HardhatRuntimeEnvironment, reserveSymbolsToSetup?: string[]): Promise<void> {
  ...
  for (const symbol of targetReserveSymbols) {
    ...
    initInputParams.push({
      ...
      aTokenName: `Palomino ${tokenName}`,
      aTokenSymbol: `Palomino-${symbol}`,
      variableDebtTokenName: `Palomino Variable Debt ${symbol}`,
      variableDebtTokenSymbol: `Palomino-variableDebt-${symbol}`,
      stableDebtTokenName: `Palomino Stable Debt ${symbol}`,
      stableDebtTokenSymbol: `Palomino-stableDebt-${symbol}`,
      params: "0x10",
    });
